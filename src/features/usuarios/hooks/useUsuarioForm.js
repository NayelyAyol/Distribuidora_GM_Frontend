...
import { registrarVendedor, registrarCliente } from "../services/vendedorService";
...
export default function useUsuarioForm(onSuccess, tipo = "VENDEDOR") {
    ...
    const handleSubmit = async (e) => {
        ...
        if (
            !onlyLetters(form.nombre) ||
            !onlyLetters(form.apellido) ||
            (isCliente && (!form.ciudad || !onlyLetters(form.ciudad)))
        ) {
        ...
        const { confirmPassword, ciudad, ...base } = form;
        ...
        toast.success(tipo === "VENDEDOR" ? "Vendedor creado" : "Cliente creado");
        ...
        toast.error(error?.message || `Error al registrar ${tipo.toLowerCase()}`);
        } finally {
        setLoading(false);
        }
    };
