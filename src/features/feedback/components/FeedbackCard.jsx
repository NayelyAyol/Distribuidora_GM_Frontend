...
export default function FeedbackCard({ item, onOpen }) {
    return (
        <div className="w-full max-w-full overflow-hidden p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm bg-emerald-700/5 hover:shadow-md transition">
            ...
            <p className="text-gray-800 text-sm sm:text-base break-words [overflow-wrap:anywhere] min-w-0">
                {item.mensaje}
            </p>

            {item.respuestaAdmin && (
                <p className="text-sm text-emerald-700 mt-2 break-words [overflow-wrap:anywhere] min-w-0">
                    Administrador: {item.respuestaAdmin}
                </p>
            )}

            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mt-4 text-sm text-gray-500 min-w-0">

                <span className="break-all text-xs sm:text-sm min-w-0">
